...
- Provide a robust async IK request wrapper with jump guards.
...
class MotionContext:
    ...
    def __init__(self, node, joint_order: List[str], vel_eps: float, require_stationary: bool):
        qos = QoSProfile(depth=10, reliability=ReliabilityPolicy.RELIABLE, history=HistoryPolicy.KEEP_LAST, durability=DurabilityPolicy.VOLATILE)
        self._node = node
...
class TrajectoryPublisher:
    ...
    def publish_positions(self, positions: List[float], move_time: float):
        ...
        traj.points = [pt]
        self._pub.publish(traj)
        self._node.get_logger().info("Published joint goal: [" + ", ".join(f"{v:.6f}" for v in positions) + f"], T={move_time:.1f}s")

class IKClient:
    """Async MoveIt IK client with jump-guarding and reference-angle wrapping."""
    ...
    def request_async(self, pose: PoseStamped, seed: JointState,
                      ref_positions: List[float],
                      done: Callable[[List[float]], None]):
        ...
        req.ik_request.timeout = MsgDuration(sec=int(self._timeout), nanosec=int((self._timeout % 1.0) * 1e9))
        ...
        def _cb(f):
            ...
            ref = {n: float(a) for n, a in zip(self._joint_order, ref_positions)}
            targets, jumps = [], []
            missing = []
            for jn in self._joint_order:
                if jn not in name_to_idx:
                    missing.append(jn); continue
                raw = float(res.solution.joint_state.position[name_to_idx[jn]])
                near = wrap_to_near(raw, ref.get(jn, raw))
                targets.append(near)
            ...
            if max_jump > self._max_warn:
                self._node.get_logger().warn(f"Abnormal joint jump (max={max_jump:.3f} > {self._max_warn:.3f}). Skipped.")
                return
            if max_jump > self._max_safe:
                self._node.get_logger().warn(f"Large joint jump (max={max_jump:.3f} > {self._max_safe:.3f}). Publishing anyway.")
